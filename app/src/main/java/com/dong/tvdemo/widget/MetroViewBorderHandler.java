...
public class MetroViewBorderHandler implements IMetroViewBorder {
    ...
    protected List<Animator> getScaleAnimator(View view, boolean isScale) {
        ...
        if (!mScalable)
            return animatorList;//如果没有放大，直接返回
        try {
    ...
    protected List<Animator> getMoveAnimator(View newFocus, int factorX, int factorY) {
        ...
        try {
            ...
            final ObjectAnimator scaleAnimator = ObjectAnimator.ofPropertyValuesHolder(mTarget, valuesWidthHolder, valuesHeightHolder, valuesYHolder,
                    valuesXHolder);
        ...
        try {
            ...
            if (oldFocus != null && newFocus != null) {
                if (oldFocus.getParent() != newFocus.getParent()) {
                    if ((attacheViews.indexOf(newFocus.getParent()) < 0) || (attacheViews.indexOf(oldFocus.getParent()) < 0 && attacheViews.indexOf(newFocus
                            .getParent()) > 0)) {
                        mTarget.setVisibility(View.INVISIBLE);
                        AnimatorSet animatorSet = new AnimatorSet();
